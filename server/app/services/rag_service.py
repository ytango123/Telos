...
class RAGService:
    """RAG indexing and search. Heavy work runs in threads so it does not block the asyncio event loop."""
    ...
    async def index_document(
        ...
        """Index a document for RAG retrieval (CPU-heavy work off the event loop)."""
    ...
    def _search_sync(self, block_id: str, query: str, top_k: int) -> List[dict]:
        table_name = f"block_{block_id}"
    ...
    async def search(
        ...
        """Search for relevant chunks (embedding + Lance run off the event loop)."""
        return await asyncio.to_thread(self._search_sync, block_id, query, top_k)
